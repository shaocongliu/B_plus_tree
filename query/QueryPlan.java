...
public class QueryPlan {
  ...
  private QueryOperator pushDownSelects(QueryOperator source, int except) throws QueryPlanException, DatabaseException {
    /* TODO: Implement me! */
    for (int i = 0; i < this.selectDataBoxes.size(); i++) { 
      if(i!=except) {
        try {
          source = new SelectOperator(source, this.selectColumnNames.get(i), this.selectOperators.get(i), this.selectDataBoxes.get(i));
  ...
  private QueryOperator minCostSingleAccess(String table) throws DatabaseException, QueryPlanException {
    ...
    // TODO: Implement me!
    SequentialScanOperator seqScanOpt = new SequentialScanOperator(this.transaction, table);
    minCost = seqScanOpt.getIOCost();
    minOp = seqScanOpt;

    // For each eligible index column, find the cost of an index scan of the
    // table and retain the lowest cost operator
    // TODO: Implement me!
    QueryOperator nextOpt;
    int nextCost;
    List<Integer> selectIndices = this.getEligibleIndexColumns(table);
    int minSelectIdx = -1;
    for (int i : selectIndices) {
      nextOpt = new IndexScanOperator(this.transaction, table, this.selectColumnNames.get(i), this.selectOperators.get(i), this.selectDataBoxes.get(i));
      nextCost = nextOpt.getIOCost();
      if(minCost > nextCost){
        minOp = nextOpt;
        minCost = nextCost;
        minSelectIdx = i;
  ...
  private QueryOperator minCostJoinType(QueryOperator leftOp,
    ...
    QueryOperator opt = null;                                                       
    QueryOperator minOp = null;
    int curr_cost;
    for (JoinOperator.JoinType operator : JoinOperator.JoinType.values()) {
      if (operator == JoinOperator.JoinType.SNLJ) {
        opt = new SNLJOperator(leftOp, rightOp, leftColumn, rightColumn, this.transaction);
      } else if (operator == JoinOperator.JoinType.GRACEHASH) {
        opt = new GraceHashOperator(leftOp, rightOp, leftColumn, rightColumn, this.transaction);
      } else if (operator == JoinOperator.JoinType.PNLJ) {
        opt = new PNLJOperator(leftOp, rightOp, leftColumn, rightColumn, this.transaction);
      } else if (operator == JoinOperator.JoinType.BNLJ) {
        opt = new BNLJOperator(leftOp, rightOp, leftColumn, rightColumn, this.transaction);
      }
      curr_cost = opt.getIOCost();
  ...
  private Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
                                               Map<Set, QueryOperator> pass1Map) throws QueryPlanException,
                                                                                        DatabaseException {
    Map<Set, QueryOperator> map = new HashMap<Set, QueryOperator>();
    QueryOperator minOpt;
    Set<String> setTable;
    String right;
    String left;
    int size = this.joinRightColumnNames.size();
    String leftColName;
    String rightColName;
    QueryOperator first;
    QueryOperator second;
    for(Set table : prevMap.keySet()) {
      for (int i = 0; i<size; i++) {
        leftColName = this.joinLeftColumnNames.get(i);
        rightColName = this.joinRightColumnNames.get(i);
        left = leftColName.split("\\.")[0];
        right = this.joinTableNames.get(i);
        setTable = new HashSet<String>();
        if ((table.contains(left)) && (!table.contains(right))) {
          setTable.add(right);
          first = prevMap.get(table);
          second = pass1Map.get(setTable);
          minOpt = this.minCostJoinType(first, second, leftColName, rightColName);
        } else {
          continue;
        }
        setTable.addAll(table);
        if ((map.get(setTable) == null)||(minOpt.getIOCost() < map.get(setTable).getIOCost())) {
          map.put(setTable, minOpt);
        }
      }
    }
    return map;
